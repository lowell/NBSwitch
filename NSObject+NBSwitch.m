...
+ (BOOL)resolveInstanceMethod:(SEL)aSEL
{
    ...
    if(hasDefaultBlock && strlen(selector)%strlen(CASE__) != strlen(DEFAULT_)-strlen(CASE__) )
        return NO;
    ...
    if(hasDefaultBlock && memcmp(selector + caseCount * strlen(CASE__), DEFAULT_, strlen(DEFAULT_)))
            return NO;
    ...
    if(hasDefaultBlock)
        [types appendFormat:@"%s",
                            @encode(void(^)(void))];

    class_addMethod(self, aSEL, (IMP)nb_switch, [types cStringUsingEncoding:NSUTF8StringEncoding]);
    return YES;
}
